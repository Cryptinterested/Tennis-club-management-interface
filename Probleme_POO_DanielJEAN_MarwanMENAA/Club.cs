...
namespace Probleme_POO_DanielJEAN_MarwanMENAA
{
    public class Club : IClub
    {
        ...
        public List<Cours> Cours { get { return Cours; } }
        ...
        */
        ...
        public string AfficheLoisir()  // affiche les membres qui sont inscrits en loisir
        {
            ...
            for (int i = 0; i < membres.Count; i++)
            {
                if (membres[i] is MembreLoisir)  res += membres[i].ToString() + "\n ";
            }
        ...
        public string AfficheCompet() // affiche les membres qui fnt de la competition
        {
            ...
            for (int i = 0; i < membres.Count; i++)
            {
                if (membres[i] is MembreCompete) res += membres[i].ToString() + "\n ";
            }
            return res;
        }
        ...
        {
            String res = "";
            List<Membre> classes = membres.FindAll(x => x is MembreCompete);
            for (int i = 0; i < classes.Count; i++) res += membres[i].ToString() + "\n ";
